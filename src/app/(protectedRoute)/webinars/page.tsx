...
const WebinaPage = async (props: Props) => {

  const checkUser = await onAuthenticate();
  if (!checkUser.user) {
    redirect('/');
  }
    ...
    >
      ...
      <TabsContent
        ...
        {webinars?.length > 0 ? (
          ...
          <div className='w-full h-[200px] flex justify-center items-center text-primary font-semibold text-2xl col-span-2'>
            No webinars found
          </div>
        )}
      </TabsContent>
